/**
 * Tesseract MCP — shared server factory.
 *
 * Loads the manifest / icons / design.md once, and exposes createServer() which
 * returns a fully-configured McpServer with all tools registered. Both the stdio
 * entry (server.mjs) and the HTTP entry (http-server.mjs) use this, so the tool
 * set can never drift between transports.
 */
...
export function createServer() {
  ...
  server.tool(
    ...
    async ({ layer = "all" }) => {
      const items = manifest.components
        ...
        .map((c) => ({ name: c.name, layer: c.layer, exports: c.exports, purpose: (c.description || "").split(/[.•\n]/)[0].trim().slice(0, 140) || null, propCount: c.props.length }));
  ...
  server.tool(
    ...
    async ({ name }) => {
      ...
      return text({
        name: c.name, layer: c.layer, exports: c.exports, description: c.description, import: c.importExample,
        props: c.props, stories: c.stories, sourcePath: c.sourcePath, storiesPath: c.storiesPath,
  ...
  server.tool(
    ...
    { component: z.string().describe("Component name"), props: z.record(z.any()).describe("The props you intend to set, as an object, e.g. { variant: 'soft', color: 'success' }") },
    async ({ component, props }) => {
      ...
      for (const [key, value] of Object.entries(props || {})) {
        ...
        if (!spec) { if (isPassthrough(key)) continue; issues.push({ prop: key, problem: "unknown prop — not in the component API", didYouMean: closest(key, c.props.map((p) => p.name)) }); continue; }
  ...
  server.tool(
    ...
    { query: z.string().optional().describe("Substring to search icon names, or an exact name to validate (e.g. 'calendar', 'heart')"), limit: z.number().optional().describe("Max names to return (default 40)") },
    async ({ query, limit = 40 }) => {
      if (!query) return text({ variants: icons.variants, curatedDefaultVariants: icons.curatedVariants, libraryIconCount: icons.count, usage: 'TPLibraryIcon name="<name>" variant="linear" size={20}  ·  active/selected → variant="bulk"', note: "Call get_icons with a query to search/validate names. Names are the authoritative set from the icon library." });
      const q = query.toLowerCase();
