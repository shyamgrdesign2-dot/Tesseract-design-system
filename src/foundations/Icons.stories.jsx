...
const Tile = ({ children, label }) => (
  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "14px 8px", border: "1px solid var(--tesseract-slate-100)", borderRadius: 10, color: "var(--tesseract-slate-700)" }}>
    {children}
    <span style={{ font: "400 10px/13px Inter", color: "var(--tesseract-slate-500)", textAlign: "center", maxWidth: "100%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", width: "100%" }}>{label}</span>
  </div>
);
    ...
    <Page>
      ...
      <div style={{ display: "flex", gap: 24, alignItems: "flex-end" }}>
        {[16, 20, 24, 32, 40].map((sz) => (
          <div key={sz} style={{ textAlign: "center", color: "var(--tesseract-slate-700)" }}>
            <TPLibraryIcon name="calendar-1" size={sz} />
            <div style={{ font: "400 10px/13px Inter", color: "var(--tesseract-slate-500)", marginTop: 6 }}>{sz}px</div>
          </div>
        ))}
        {["--tesseract-blue-500", "--tesseract-success-600", "--tesseract-warning-600", "--tesseract-error-600"].map((c) => (
          <div key={c} style={{ textAlign: "center", color: `var(${c})` }}>
            <TPLibraryIcon name="heart-add" size={28} />
            <div style={{ font: "400 10px/13px Inter", color: "var(--tesseract-slate-500)", marginTop: 6 }}>{c.replace("--tesseract-", "")}</div>
          </div>
        ))}
      </div>
    </Page>
...
export const Usage = {
  ...
  render: () => (
    <Page>
      ...
      <div style={{ font: "600 14px/20px ui-monospace, Menlo, monospace", color: "var(--tesseract-slate-700)", background: "var(--tesseract-slate-50)", border: "1px solid var(--tesseract-slate-200)", borderRadius: 10, padding: "12px 14px", marginBottom: 12 }}>
        &lt;base&gt; / &lt;corner&gt; / &lt;style&gt; / &lt;family&gt; / &lt;name&gt;.svg
      </div>
      <div style={{ display: "grid", gap: 6, marginBottom: 8 }}>
        {PATH_PARTS.map((p) => (
          <div key={p.seg} style={{ display: "grid", gridTemplateColumns: "120px 150px 1fr", gap: 12, alignItems: "baseline", font: "400 13px/18px Inter", color: "var(--tesseract-slate-600)" }}>
            <code style={{ color: "var(--tesseract-blue-600)", fontWeight: 600 }}>{p.seg}</code>
            <code style={{ color: "var(--tesseract-slate-800)" }}>{p.val}</code>
            <span>{p.desc}</span>
          </div>
        ))}
      </div>
      ...
      <pre style={{ font: "500 12px/18px ui-monospace, Menlo, monospace", background: "var(--tesseract-slate-900)", color: "#e6e6f0", padding: 16, borderRadius: 12, overflowX: "auto", margin: "0 0 8px" }}>{`import { TPLibraryIcon } from "@/src/components/atoms/icons/tp/TPLibraryIcon";
...
<TPLibraryIcon name="heart-add" variant="bulk" size={24} color="var(--tesseract-error-600)" />
...
<TPLibraryIcon name="bold/trash" size={20} />`}</pre>
      <div style={{ display: "flex", gap: 16, alignItems: "center", margin: "6px 0 4px" }}>
        ...
        <Tile label="bulk"><TPLibraryIcon name="heart-add" variant="bulk" size={24} color="var(--tesseract-error-600)" /></Tile>
        <Tile label="bold/trash"><TPLibraryIcon name="bold/trash" size={24} /></Tile>
      </div>

      <Section>Semantic chevrons</Section>
      <p style={{ font: "400 14px/22px Inter", color: "var(--tesseract-slate-600)", maxWidth: 720, margin: "0 0 12px" }}>
