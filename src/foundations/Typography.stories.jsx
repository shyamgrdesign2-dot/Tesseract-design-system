...
function Row({ cls, family, weight, size, lh }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "150px 1fr 230px", gap: 16, alignItems: "baseline", padding: "12px 0", borderBottom: "1px solid var(--tp-slate-100)" }}>
      <code style={{ font: "500 12px/16px ui-monospace, monospace", color: "var(--tp-slate-600)" }}>.tp-{cls}</code>
      <span className={`tp-${cls}`}>The five boxing wizards jump quickly</span>
      <span style={{ font: "400 11px/16px Inter", color: "var(--tp-slate-500)" }}>{family} · {weight} · {size}/{lh}</span>
    </div>
  );
}
...
export const Scale = {
  render: () => (
    <Page>
      <Title sub={<>Headings use <strong>Mulish</strong>; body, labels, captions and code use <strong>Inter</strong> / mono. Apply a role as a class, e.g. <Code>&lt;p class="tp-body-base"&gt;</Code>, or compose from the size/line-height tokens (<Code>--tp-text-h1</Code> / <Code>--tp-text-h1-lh</Code>).</>}>Typography</Title>
  ...
  render: () => (
    <Page>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 24 }}>
        {[["Mulish", "var(--tp-font-heading)", "Headings (display + h1–h6)"], ["Inter", "var(--tp-font-body)", "Body, labels, captions, UI"]].map(([name, fam, use]) => (
          <div key={name} style={{ border: "1px solid var(--tp-slate-200)", borderRadius: 12, padding: 20 }}>
            ...
            <div style={{ fontFamily: fam, fontSize: 13, color: "var(--tp-slate-500)", marginTop: 4 }}>ABCDEFGHIJKLM · abcdefghijklm · 0123456789</div>
            <div style={{ marginTop: 12, font: "600 13px/16px Inter" }}>{name}<span style={{ font: "400 12px/16px Inter", color: "var(--tp-slate-500)" }}> — {use}</span></div>
          </div>
        ))}
      </div>
    </Page>
