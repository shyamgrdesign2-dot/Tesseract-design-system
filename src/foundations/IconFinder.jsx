...
export function IconFinder() {
  ...
  return (
    <div style={{ fontFamily: "Inter, sans-serif", color: "var(--tesseract-slate-900)", padding: 28, maxWidth: 1120 }}>
      ...
      <p style={{ font: "400 14px/22px Inter", color: "var(--tesseract-slate-500)", maxWidth: 760, margin: "0 0 22px" }}>
      ...
      <div style={{ display: "grid", gap: 14, marginBottom: 20 }}>
        ...
        <div style={{ font: "400 13px/18px Inter", color: "var(--tesseract-slate-500)" }}>
          <strong style={{ color: "var(--tesseract-slate-800)" }}>{matches.length.toLocaleString()}</strong> {matches.length === 1 ? "icon" : "icons"} for {corner}/{style}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(96px, 1fr))", gap: 8 }}>
          {shown.map((n) => (
            <button key={n} type="button" onClick={() => copy(n)} title={`Copy CDN URL — ${getIconBaseUrl()}/${iconPath({ name: n, style, corner })}`}
              style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, padding: "16px 6px", border: "1px solid var(--tesseract-slate-100)", borderRadius: 10, background: copied === n ? "var(--tesseract-blue-50)" : "var(--tesseract-slate-0)", color: copied === n ? "var(--tesseract-blue-600)" : "var(--tesseract-slate-700)", cursor: "pointer", transition: "background 120ms ease, border-color 120ms ease" }}>
              <TPLibraryIcon name={n} variant={style} corner={corner} size={26} />
              <span style={{ font: "400 10px/13px Inter", color: copied === n ? "var(--tesseract-blue-600)" : "var(--tesseract-slate-500)", textAlign: "center", maxWidth: "100%", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", width: "100%" }}>
