    ...
    docs: {
      description: {
        component: [
          ...
          '',
          '**When to use** — choosing a magnitude where the position itself is meaningful: a pain score, a dosage, a volume or threshold filter. Pair with `showValue` when the exact figure matters.',
          '**When not** — a small fixed set of mutually-exclusive choices is `SegmentedControl` or `Radio`; a free-form number is `InputBox type="number"`.',
          '',
          '**Key props** — `value` + `onChange` (controlled) or `defaultValue` (uncontrolled); `min` / `max` / `step` bound and quantise the scale; `marks` shows ticks (`true` = min & max, or an array of `{value,label}`); `showValue` + `formatValue` render the live readout; `color` maps to a tone; `error` flags an out-of-range value.',
          '',
          '**Good to know** — `onChange` is `(event, value)`, value second. The thumb is keyboard-driven (arrows step, Home/End jump) with a `focus-visible` ring, so always give it a `label` for screen readers.',
        ].join('\n'),
      },
    },
  },
  argTypes: {
    color: { control: 'inline-radio', options: ['primary', 'success', 'warning', 'error'], description: 'Track + thumb tone' },
    size: { control: 'inline-radio', options: ['sm', 'md', 'lg'], description: 'Track thickness / thumb size' },
    min: { control: 'number', description: 'Lower bound of the scale' },
    max: { control: 'number', description: 'Upper bound of the scale' },
    step: { control: 'number', description: 'Quantises the value; use a fraction for decimals' },
