...
const meta = {
  ...
  parameters: {
    docs: { description: { component: [
      'A compact, transient notification — a status icon, a title, optional subtext, an optional `Button` action, and a dismiss control.',
      '',
      ...
      '',
      '**Key props** — `status` (`info` | `success` | `warning` | `error` — colours the icon only); `title` + `children` (subtext); `surface` (`dark` default | `light`); `action` (a `Button` whose `surface` should match the toast); `dismissible` (the × ); `duration` + `progress` (auto-dismiss timer with an optional depleting bar); `icon`/`showIcon`.',
      '',
      '**Good to know** — line rules keep height predictable: title-only wraps up to 2 lines then truncates; title + subtext each clamp to one truncated line. The auto-dismiss timer pauses on hover; `aria-live` announces it.',
    ].join('\n') } },
  },
  argTypes: {
    status:      { control: 'inline-radio', options: ['info', 'success', 'warning', 'error'], description: 'Semantic tone — colours the status icon', table: { category: 'Content' } },
    ...
    title:       { control: 'text', description: 'Primary message line', table: { category: 'Content' } },
