...
| **Component package** | `src/` → `@dhspl-tatvacare/tesseract-ui` | **Private GitHub Packages** (published from a GitHub Release via `publish-package.yml`, built-in `GITHUB_TOKEN`). Consumers add a one-time `.npmrc` + `read:packages` token, then `npm install` / `npm update` within `1.x`. |
