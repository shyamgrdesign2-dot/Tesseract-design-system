...
**Start from the default page skeleton** in `references/page-archetypes.md` (read it now) — four parts: **`Sidebar` → `Header` (standard: back + title + right actions) → `HeroBanner` (carries the H1) → an overlay `Card`/`SectionCard` that OVERLAPS the banner** (top edge sits on the banner, then runs down), holding the toolbar (`InputBox` search + `Filter`) and the archetype body. **Reproduce this frame by default** — even for a vague "build me a page" request — and change only the card body per archetype; don't invent a different layout. Add a `SecondarySidebar` sub-nav (record pages) or a right-docked **Dr. Agent** `Drawer` (clinical/ops) **only when the page needs them** — not on a plain page. Then read `references/component-catalog.md` and, for each region, pick the Tesseract component that fills it (molecules for regions, atoms for leaves).
